...
use App\Mail\KonfirmasiStatusKematian;
...
class EditSuratKematian extends EditRecord
{
    ...
    public function kirimEmailKonfirmasi($status, $catatan)
    {
        ...
        if ($warga && $warga->email) {
            Mail::to($warga->email)->send(new KonfirmasiStatusKematian([
                'nama' => $warga->name,
            ...
            Mail::to('gilangelha277@gmail.com')->send(new KonfirmasiStatusKematian([
                'nama' => 'Admin',
                'status' => ucfirst($status),
