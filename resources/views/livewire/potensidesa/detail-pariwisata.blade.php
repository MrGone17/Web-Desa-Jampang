<div>
    <section class="relative bg-cover bg-center bg-no-repeat" style="background-image: url('{{ asset('storage/' . $pariwisata->image) }}');">
        <div class="absolute inset-0 bg-gray-900 bg-opacity-70"></div>
        <div class="relative container mx-auto px-6 md:px-20 py-16">
            <div class="text-center md:text-start text-white">
                <h2 class="text-3xl md:text-5xl font-bold">Pariwisata Desa Jampang</h2>
            </div>
        </div>
    </section>
    <div class=" bg-white">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-16">
                ...
                <div class="col-span-1 md:col-span-9 grid">
                    ...
                    <div class="py-2 text-sm md:text-lg text-gray-500 order-2">
                        {{ \Carbon\Carbon::parse($pariwisata->publish_date)->format('d F Y') }}
                    </div>
    
                    <div class="rounded-xl overflow-hidden order-1 my-5 md:order-3 cursor-pointer"
                        onclick="openImageModal('{{ asset('storage/' . $pariwisata->image) }}','{{ $pariwisata->title }}')">
                        <img src="{{ asset('storage/' . $pariwisata->image) }}" alt="{{ $pariwisata->title }}"
                ...
                <div class="col-span-1 md:col-span-3">
                    <div class="bg-white shadow-xl p-4 rounded-md">
                        <div class="mb-5 font-bold">pariwisata Lainnya</div>
    
                        @foreach ($pariwisataLain as $item)
                            <a href="{{ route('detail-pariwisata', ['slug' => $item->slug]) }}" class="block w-full mb-4 rounded-xl">
