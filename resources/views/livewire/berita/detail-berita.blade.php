<div>
    <section class="relative bg-cover bg-center bg-no-repeat" style="background-image: url('{{ asset('storage/' . $berita->image) }}');">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-16">
                ...
                <div class="col-span-1 md:col-span-9 grid">
                    ...
                    <div class="py-2 text-sm md:text-lg text-gray-500 order-2">
                        {{ \Carbon\Carbon::parse($berita->publish_date)->format('d F Y') }}
                    </div>
    
                    <div class="rounded-xl overflow-hidden order-1 my-5 md:order-3 cursor-pointer"
                        onclick="openImageModal('{{ asset('storage/' . $berita->image) }}','{{ $berita->title }}')">
                ...
                <div class="col-span-1 md:col-span-3">
                    <div class="bg-white shadow-xl p-4 rounded-md">
                        ...
                        @foreach ($beritaLain as $item)
                            <a href="{{ route('detail-berita', ['slug' => $item->slug]) }}" class="block w-full mb-4 rounded-xl">
